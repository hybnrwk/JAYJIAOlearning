...
> **材料依据：** 《高维数据分析》第一讲《散点图与回归》，袁洪松，lecture01，第 1—4 页；Sanford Weisberg, *Applied Linear Regression*, 4th ed., Chapter 1, “Scatterplots and Regression”，第 1—20 页。  
> **学习定位：** 中文负责解释，英文用于标注术语；以课件主线为骨架，用教材案例补足图形判断、模型诊断与多变量视角。

**阅读顺序：** 先沿第 1—8 部分理解“回归问题—散点图—均值函数—方差函数—诊断工具”；第 9—12 部分用于案例串联、解题和复习；文末附术语索引与原文对应表。

**标记说明：** “课件主线”表示 lecture01 明确讲授的内容；“教材例证”表示 Weisberg Chapter 1 的案例与图形；“理解补充”表示为解释逻辑加入的内容。
...
\longrightarrow
\begin{cases}
\text{看中心：mean function},\\
\text{看散布：variance function},\\
\text{看特殊点：outlier / leverage}
\end{cases}
\longrightarrow
\text{拟合、诊断、检验或预测}.
$$
...
## 6. 特殊点：离群点（outlier）与杠杆点（leverage point）不能混
